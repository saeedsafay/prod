    ...
    <div class="md-card  uk-margin-small-top">
        <div class="md-card-content">
            <div class="uk-width-medium-1-6 uk-margin-small-bottom">
                <a class="md-btn md-btn-success" href="{site_url({ADMIN_PATH|con:"/shop/product-types/edit"})}">افزودن گروه کالا</a>
            </div>
        </div>
    </div>
    <div class="md-card uk-margin-medium-bottom">
        <div class="md-card-content">
            <table class="uk-table dataTable uk-table-striped" id="dt_default" role="grid" aria-describedby="dt_default_info">
                <thead>
                    <tr>
                        <th class="sorting">عنوان گروه کالایی </th>
                ...
                <tbody class="uk-table uk-table-striped">
                    {foreach from=$Product_types item=val}
                        <tr>
                            <td>{$val->title}</td>
                            <td>{$val->slug}</td>
                            ...
                            <td class="right">
                                <a class="md-btn md-btn-primary md-btn-small" href="{site_url({ADMIN_PATH|con:'/shop/product-types/edit/' : $val->id})}">ویرایش</a>
                                <a href="{site_url({ADMIN_PATH|con:'/shop/product-types/delete/':$val->id})}" class="md-btn md-btn-small md-btn-danger delete" id="delete-btn">حذف گروه کالایی</a>
                            </td>
                        </tr>
                    {/foreach}
                </tbody>
