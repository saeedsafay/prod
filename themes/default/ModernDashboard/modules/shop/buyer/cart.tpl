            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
                <form action="#">
                    <div class="table-content table-responsive">
                        ...
                        {else}
                        <h4 class="title_row">سبد خرید شما</h4><br>
                        <hr>
                        <br>
                        <table>
                            <thead>
                            ...
                            {foreach from=$CartItems item=Item}
                                <tr id="cartItem{$Item.id}">
                                    <td class="product-thumbnail"><a href="/product/npi-{$Item.id}/{$Item.slug}">
                                            <img src="{site_url()|con:'upload/ads/pic/':$Item.pic}"
                                                 alt="{$Item.title}"/>
                                        </a>
                                    </td>
                                    <td class="">
                                    ...
                                    <td class="product-remove"><a href="#"
                                                                  onclick="$(this).removeCartItem({$Item.id});">X</a>
                                    </td>
                                </tr>
                                        ...
                                        <td>
                                            <ul id="shipping_method">
                                                ...
                                                {foreach from=$shippings item=shipping}
                                                    <li>
                                                        <input id="shipping{$shipping.id}" type="radio"
                                                               name="shipping"/>
                                                        <label for="shipping{$shipping.id}">
                                                            {$shipping.shipping} <span
                                                                    class="amount">{$shipping.price}</span>
                                                        </label>
                                                    </li>
                                                {/foreach}
                                            </ul>
