...
class Entries extends Admin_Controller
{
     ...
     * @param $type
     */
    public function entry_list($type)
    {
    ...
    public function entry_edit($type, $method, $id = null)
    {
        ...
        $this->load->eloquent('shop/Product_category');
        $content_type = Content_type::findBySlug(urldecode($type));
        ...
        $this->smart->assign([
            ...
            'Product_categories' => Product_category::whereNotNull('parent_id')->get(),
        ...
        if ($this->formValidate(true, $method, $Extra_fields)) {
            ...
            if ($entry->save()) {
                ...
                $entry->product_categories()->sync($this->input->post('product_categories'));
                ...
                if ( ! $edit_mode) {
                    $this->message->set_message('اطلاعات با موفقیت ذخیره شد', 'success', 'ثبت رکورد جدید',
                        ADMIN_PATH.'/content/entries/'.$type.'/')->redirect();
                } else {
                ...
                if ($edit_mode) {
                    $this->message->set_message('ذخیره سازی انجام نشد. مجدد تلاش کنید', 'fail', 'خطا در ذخیره سازی',
                        ADMIN_PATH.'/content/entries/'.$type.'/'.$method.'/'.$id)->redirect();
                } else {
                    $this->message->set_message('بروزرسانی انجام نشد. مجدد تلاش کنید', 'fail', 'خطا در  بروزسانی',
                        ADMIN_PATH.'/content/entries/'.$type.'/'.$method.'/')->redirect();
                }
            }
