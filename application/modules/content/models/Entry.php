...
require_once FCPATH . 'application/modules/shop/models/Category.php';
...
class Entry extends Illuminate\Database\Eloquent\Model {
    ...
    {
        return $this->belongsToMany('Content_category' , 'content_category_content_entries' , 'content_entry_id');
    }

    public function product_categories()
    {
        return $this->belongsToMany('Category' , 'content_product_categories' , 'content_entry_id' , 'product_category_id');
    }
    ...
    {
        if( ($value = $this->fields->where('content_type_field_type_id' , $content_type_field_type_id)->first() ) ) {
    ...
    public function scopeExclude( $query , $excludes )
    {
        foreach( $excludes as $exclude ) {
            ...
            if( strpos($exclude , '-') ) {
                ...
                $query->whereNotBetween('id' , array( $rangeSides[0] , $rangeSides[1] ));
            }
            else {
        ...
        return $query->where(function($query) use ($posts) {
                    foreach( $posts as $post ) {
                        ...
                        if( strpos($post , '-') ) {
                            ...
                            $query->orWhereBetween('id' , array( $rangeSides[0] , $rangeSides[1] ));
                        }
                        else {
