...
class Migrations
{
    ...
    public function migration_do($module)
    {
        ...
        $repo = new MirookMigrationRepository(Illuminate\Database\Eloquent\Model::getConnectionResolver(),
            'migrations');
        $migrator = new MirookMigrator($repo, Illuminate\Database\Eloquent\Model::getConnectionResolver(),
            $file_system);
...
class MirookMigrator extends \Illuminate\Database\Migrations\Migrator
{
    ...
    public function mirook_back_by_batch($path, $pretend = false)
    {
        ...
        $files = array_reverse(array_intersect($intersectFiles, $ran));
        if (count($files) == 0) {
            $this->note('<info>Nothing to migrate down.</info>');
            dd('Nothing to migrate down');
            return;
        }
        $migrations = array();
        foreach ($files as $key => $file) {
    ...
    public function downMigrationList($migrations, $pretend = false)
    {
        ...
        foreach ($migrations as $migration) {
            $this->runDown($migration->migration, (object)$migration, $pretend);
        }
    }
