...
class nusoap_base {
     ...
     * @access	public
     */
    function serialize_val($val, $name = false, $type = false, $name_ns = false, $type_ns =
    false, $attributes = false, $use = 'encoded', $soapval = false) {
        ...
        switch (true) {
            ...
            case is_object($val):
                ...
                if (get_class($val) == 'soapval') {
                    ...
                    foreach (get_object_vars($val) as $k => $v) {
                        $pXml = isset($pXml) ? $pXml . $this->serialize_val($v,
                                        $k, false, false, false, false, $use) : $this->serialize_val($v,
            ...
            case (is_array($val) || $type):
                ...
                if ($valueType == 'arraySimple' || preg_match('/^ArrayOf/',
                                $type)) {
                    ...
                    if (is_array($val) && count($val) > 0) {
                        foreach ($val as $v) {
                            ...
                            $xml .= $this->serialize_val($v, 'item', false,
                    ...
                    foreach ($val as $k => $v) {
                        ...
                        if ($type == 'Map' && $type_ns == 'http://xml.apache.org/xml-soap') {
                            $xml .= '<item>';
                            $xml .= $this->serialize_val($k, 'key', false,
                                    false, false, false, $use);
                            $xml .= $this->serialize_val($v, 'value', false,
                                    false, false, false, $use);
                            $xml .= '</item>';
                        } else {
    ...
    'rpc', $use = 'encoded', $encodingStyle = 'http://schemas.xmlsoap.org/soap/encoding/') {
        ...
        $this->debug("In serializeEnvelope length=" . strlen($body) . " body (max 1000 characters)=" . substr($body,
                        0, 1000) . " style=$style use=$use encodingStyle=$encodingStyle");
        $this->debug("headers:");
        ...
        if ($headers) {
            if (is_array($headers)) {
                ...
                foreach ($headers as $k => $v) {
                    if (is_object($v) && get_class($v) == 'soapval') {
                        $xml .= $this->serialize_val($v, false, false, false,
                                false, false, $use);
                    } else {
                        $xml .= $this->serialize_val($v, $k, false, false,
                                false, false, $use);
...
function timestamp_to_iso8601($timestamp, $utc = true) {
    ...
    if ($utc) {
        ...
        if (preg_match($pattern, $datestr, $regs)) {
            return sprintf('%04d-%02d-%02dT%02d:%02d:%02dZ', $regs[1], $regs[2],
...
class nusoap_xmlschema extends nusoap_base {
    ...
    function parseString($xml, $type) {
        ...
        if ($xml != "") {
            ...
            if ($type == "schema") {
                xml_set_element_handler($this->parser, 'schemaStartElement',
                        'schemaEndElement');
                xml_set_character_data_handler($this->parser,
                        'schemaCharacterData');
            } elseif ($type == "xml") {
                xml_set_element_handler($this->parser, 'xmlStartElement',
                        'xmlEndElement');
    ...
    function schemaStartElement($parser, $name, $attrs) {
        ...
        if ($depth > 0) {
            $this->defaultNamespace[$pos] = $this->defaultNamespace[$this->depth_array[$depth
                    - 1]];
        } else {
                ...
                if (preg_match('/^xmlns/', $k)) {
                    ...
                    if ($ns_prefix = substr(strrchr($k, ':'), 1)) {
                        ...
                        if (!$this->getPrefixFromNamespace($v)) {
                            $this->namespaces['ns' . (count($this->namespaces) + 1)] =
                                    $v;
                        }
                    }
        ...
        switch ($name) {
            ...
            case 'attribute': // complexType attribute
                    ...
                    if (!strpos($v, ':')) {
                        ...
                        if ($this->defaultNamespace[$pos]) {
                            ...
                            $attrs['http://schemas.xmlsoap.org/wsdl/:arrayType'] =
                                    $this->defaultNamespace[$pos] . ':' . $attrs['http://schemas.xmlsoap.org/wsdl/:arrayType'];
                        }
                    }
                }
                if (isset($attrs['name'])) {
                ...
                if ($this->currentComplexType) { // This should *always* be
                    $this->complexTypes[$this->currentComplexType]['attrs'][$aname] =
                            $attrs;
                }
                // arrayType attribute
                if (isset($attrs['http://schemas.xmlsoap.org/wsdl/:arrayType']) || $this->getLocalPart($aname)
                        == 'arrayType') {
                    ...
                    if (strpos($v, '[,]')) {
                        $this->complexTypes[$this->currentComplexType]['multidimensional'] =
                                true;
                    }
            ...
            case 'complexType':
                ...
                if (isset($attrs['name'])) {
                    ...
                    $this->complexTypes[$this->currentComplexType]['typeClass'] =
                            'complexType';
                    ...
                    //                </sequence>
                    //            </complexType>
                    if (isset($attrs['base']) && preg_match('/:Array$/',
                                    $attrs['base'])) {
                ...
                $this->complexTypes[$this->currentComplexType]['simpleContent'] =
                        'false';
                break;
            case 'element':
                ...
                if (isset($attrs['type'])) {
                    ...
                    //                </sequence>
                    //            </complexType>
                    if ($this->currentComplexType && $this->complexTypes[$this->currentComplexType]['phpType']
                            == 'array') {
                ...
                if (isset($ename) && $this->currentComplexType) {
                    ...
                    $this->complexTypes[$this->currentComplexType]['elements'][$ename] =
                            $attrs;
                } elseif (!isset($attrs['ref'])) {
            ...
            case 'enumeration': //	restriction value list member
                ...
                if ($this->currentSimpleType) {
                    $this->simpleTypes[$this->currentSimpleType]['enumeration'][] =
                            $attrs['value'];
    ...
    function schemaEndElement($parser, $name) {
        ...
        if ($name == 'element') {
            $this->xdebug('done processing element ' . ($this->currentElement ? $this->currentElement
                                : '(unknown)'));
     ...
     * @see getTypeDef
     */
    function addSimpleType($name, $restrictionBase = '', $typeClass = 'simpleType', $phpType =
...
class soapval extends nusoap_base {
     ...
     * @access   public
     */
    function __construct($name = 'soapval', $type = false, $value = -1, $element_ns =
    false, $type_ns = false, $attributes = false) {
        parent::__construct();
        $this->name = $name;
        $this->type = $type;
        $this->value = $value;
    ...
    function serialize($use = 'encoded') {
        return $this->serialize_val($this->value, $this->name, $this->type,
                        $this->element_ns, $this->type_ns, $this->attributes,
                        $use, true);
    }
...
class soap_transport_http extends nusoap_base {
    ...
    function io_method() {
        if ($this->use_curl || ($this->scheme == 'https') || ($this->scheme == 'http'
            ...
            return 'curl';
        if (($this->scheme == 'http' || $this->scheme == 'ssl') && $this->authtype
                != 'ntlm' && (!is_array($this->proxy) || $this->proxy['authtype']
                != 'ntlm'))
            return 'socket';
        return 'unknown';
    }
...
//			$this->setError('cURL Extension, or OpenSSL extension w/ PHP version >= 4.3 is required for HTTPS');
            ...
            $this->ch = curl_init();
            // set url
            $hostURL = ($this->port != '') ? "$this->scheme://$this->host:$this->port"
...
//		}
            ...
            if ($this->scheme == 'https') {
                ...
                if ($this->authtype == 'certificate') {
                    ...
                    if (isset($this->certRequest['verifypeer'])) {
                        $this->setCurlOption(CURLOPT_SSL_VERIFYPEER,
                    ...
                    if (isset($this->certRequest['passphrase'])) {
                        $this->setCurlOption(CURLOPT_SSLKEYPASSWD,
                                $this->certRequest['passphrase']);
                    }
                    if (isset($this->certRequest['certpassword'])) {
                        $this->setCurlOption(CURLOPT_SSLCERTPASSWD,
     ...
     * @access   public
     */
    function setCredentials($username, $password, $authtype = 'basic', $digestRequest =
    array(), $certRequest = array()) {
        ...
        if ($authtype == 'basic') {
            ...
            if (isset($digestRequest['nonce'])) {
                $digestRequest['nc'] = isset($digestRequest['nc']) ? $digestRequest['nc'] ++
                            : 1;

                // calculate the Digest hashes (calculate code based on digest implementation found at: http://www.rassoc.com/gregr/weblog/stories/2002/07/09/webServicesSecurityHttpDigestAuthenticationWithoutActiveDirectory.html)
                // A1 = unq(username-value) ":" unq(realm-value) ":" passwd
                $A1 = $username . ':' . (isset($digestRequest['realm']) ? $digestRequest['realm']
                                    : '') . ':' . $password;
    ...
    '', $proxyauthtype = 'basic') {
        if ($proxyhost) {
            ...
            if ($proxyusername != '' && $proxypassword != '' && $proxyauthtype =
                    'basic') {
    ...
    function getResponse() {
        ...
        if ($this->io_method() == 'socket') {
            ...
            if (isset($this->incoming_headers['transfer-encoding']) && strtolower($this->incoming_headers['transfer-encoding'])
                    == 'chunked') {
            ...
            // close filepointer
            if (
                    (isset($this->incoming_headers['connection']) && strtolower($this->incoming_headers['connection'])
                    == 'close') ||
...
//		}
        ...
        if (isset($this->incoming_headers['location']) && ($http_status == 301 || $http_status
                == 302)) {
        ...
        if (isset($this->incoming_headers['www-authenticate']) && $http_status == 401) {
            ...
            if (strstr($this->incoming_headers['www-authenticate'], "Digest ")) {
                ...
                $digestString = str_replace('Digest ', '',
...
class nusoap_server extends nusoap_base {
        ...
        if (isset($debug)) {
            ...
            foreach ($qs as $v) {
                if (substr($v, 0, 6) == 'debug=') {
                    $this->debug("In nusoap_server, set debug_flag=" . substr($v,
                                    6) . " based on query string #1");
    ...
    function parse_request($data = '') {
        ...
        if (isset($this->headers['content-encoding']) && $this->headers['content-encoding']
            ...
            if ($this->headers['content-encoding'] == 'deflate' || $this->headers['content-encoding']
                    == 'gzip') {
                // if decoding works, use it. else assume data wasn't gzencoded
                if (function_exists('gzuncompress')) {
                    if ($this->headers['content-encoding'] == 'deflate' && $degzdata =
                            @gzuncompress($data)) {
                        $data = $degzdata;
                    } elseif ($this->headers['content-encoding'] == 'gzip' && $degzdata =
    ...
    function invoke_method() {
        ...
        if (strlen($delim) > 0 && substr_count($this->methodname, $delim) == 1) {
            $try_class = substr($this->methodname, 0,
                    strpos($this->methodname, $delim));
            if (class_exists($try_class)) {
                // get the class and method name
                $class = $try_class;
                $method = substr($this->methodname,
        ...
        if ($class == '') {
            ...
            $method_to_compare = (substr(phpversion(), 0, 2) == '4.') ? strtolower($method)
                        : $method;
    ...
    function serialize_return() {
        ...
        // if fault
        if (isset($this->methodreturn) && is_object($this->methodreturn) && ((get_class($this->methodreturn)
                == 'soap_fault') || (get_class($this->methodreturn) == 'nusoap_fault'))) {
            ...
            if ($this->wsdl) {
                ...
                $return_val = $this->wsdl->serializeRPCParameters($this->methodname,
                        'output', $opParams);
                $this->appendDebug($this->wsdl->getDebug());
                $this->wsdl->clearDebug();
                ...
                if (isset($this->methodreturn)) {
                    $return_val = $this->serialize_val($this->methodreturn,
                            'return');
                } else {
                    $return_val = '';
        ...
        if ($this->wsdl) {
            ...
            $this->responseSOAP = $this->serializeEnvelope($payload,
                    $this->responseHeaders, $this->wsdl->usedNamespaces,
                    $this->opData['style'], $this->opData['output']['use'],
                    $encodingStyle);
        } else {
            $this->responseSOAP = $this->serializeEnvelope($payload,
    ...
    function parseRequest($headers, $data) {
        ...
        $parser = new nusoap_parser($data, $this->xml_encoding, '',
                $this->decode_utf8);
        // parser debug
     ...
     * @access   public
     */
    function register($name, $in = array(), $out = array(), $namespace = false, $soapaction =
    false, $style = false, $use = false, $documentation = '', $encodingStyle = '') {
        global $HTTP_SERVER_VARS;
    ...
    'rpc', $transport = 'http://schemas.xmlsoap.org/soap/http', $schemaTargetNamespace =
    false) {
        global $HTTP_SERVER_VARS;
...
class wsdl extends nusoap_base {
    ...
    function __construct($wsdl = '', $proxyhost = false, $proxyport = false, $proxyusername =
    false, $proxypassword = false, $timeout = 0, $response_timeout = 30, $curl_options =
    null, $use_curl = false) {
        parent::__construct();
                ...
                foreach ($list as $xs) {
                    ...
                    foreach ($xs->imports as $ns2 => $list2) {
                        for ($ii = 0; $ii < count($list2); $ii++) {
                            if (!$list2[$ii]['loaded']) {
                                $this->schemas[$ns]->imports[$ns2][$ii]['loaded'] =
                                        true;
    ...
    function parseWSDL($wsdl = '') {
        ...
        if (isset($wsdl_props['scheme']) && ($wsdl_props['scheme'] == 'http' || $wsdl_props['scheme']
                == 'https')) {
        ...
        if (!xml_parse($this->parser, $wsdl_string, true)) {
            // Display an error message.
            $errstr = sprintf(
                    'XML error parsing WSDL from %s on line %d: %s', $wsdl,
                    xml_get_current_line_number($this->parser),
                    xml_error_string(xml_get_error_code($this->parser))
            );
            $this->debug($errstr);
                        ...
                        case 'body':
                            if (isset($this->bindings[$this->currentBinding]['operations'][$this->currentOperation][$this->opStatus])) {
                                $this->bindings[$this->currentBinding]['operations'][$this->currentOperation][$this->opStatus] =
                                        array_merge($this->bindings[$this->currentBinding]['operations'][$this->currentOperation][$this->opStatus],
                                        $attrs);
                            } else {
                                $this->bindings[$this->currentBinding]['operations'][$this->currentOperation][$this->opStatus] =
                                        $attrs;
                ...
                case 'service':
                    switch ($name) {
                        ...
                        case 'address':
                            ...
                            $this->bindings[$this->ports[$this->currentPort]['binding']]['bindingType'] =
                                    $namespace;
    ...
    function character_data($parser, $data) {
        $pos = isset($this->depth_array[$this->depth]) ? $this->depth_array[$this->depth]
                    : 0;
     ...
     * @access public
     */
    function serializeRPCParameters($operation, $direction, $parameters, $bindingType =
    'soap') {
        ...
        if (isset($opData[$direction]['parts']) && sizeof($opData[$direction]['parts'])
            ...
            if (is_array($parameters)) {
                ...
                if ($style == 'document' && $use == 'literal' && $part_count == 1
                    ...
                    if ($direction == 'output' && $parametersArrayType == 'arraySimple'
                            && $parameter_count == 1) {
                    ...
                    if (($parametersArrayType == 'arrayStruct' || $parameter_count
                            == 0) && !isset($parameters['parameters'])) {
                        $this->debug('check whether caller\'s parameters match the wrapped ones');
                        if ($this->parametersMatchWrapped($parts['parameters'],
                                        $parameters)) {
                ...
                foreach ($parts as $name => $type) {
                    ...
                    if ($parametersArrayType == 'arraySimple') {
                        ...
                        $xml .= $this->serializeType($name, $type, $p, $use,
                                $enc_style);
                    } elseif (isset($parameters[$name])) {
                        $this->debug('calling serializeType w/named param');
                        $xml .= $this->serializeType($name, $type,
                                $parameters[$name], $use, $enc_style);
                    } else {
                        // TODO: only send nillable
                        $this->debug('calling serializeType w/null param');
                        $xml .= $this->serializeType($name, $type, null, $use,
                                $enc_style);
                    }
                }
            } else {
     ...
     * @access private
     */
    function serializeType($name, $type, $value, $use = 'encoded', $encodingStyle =
    false, $unqualified = false) {
        $this->debug("in serializeType: name=$name, type=$type, use=$use, encodingStyle=$encodingStyle, unqualified=" . ($unqualified
        ...
        if (strpos($type, ':')) {
            ...
            if ($ns == $this->XMLSchemaVersion || $ns == 'http://schemas.xmlsoap.org/soap/encoding/') {
                ...
                if ($uqType == 'Array') {
                    ...
                    return $this->serialize_val($value, $name, false, false,
                                    false, false, $use);
                }
                if ($uqType == 'boolean') {
                ...
                if (($uqType == 'long' || $uqType == 'unsignedLong') && gettype($value)
                        == 'double') {
                ...
                if ($uqType == 'Map') {
                    ...
                    foreach ($value as $k => $v) {
                        ...
                        $contents .= '<item>';
                        $contents .= $this->serialize_val($k, 'key', false,
                                false, false, false, $use);
                        $contents .= $this->serialize_val($v, 'value', false,
                                false, false, false, $use);
                        $contents .= '</item>';
                    }
                    if ($use == 'literal') {
        ...
        $phpType = $typeDef['phpType'];
        $this->debug("in serializeType: uqType: $uqType, ns: $ns, phptype: $phpType, arrayType: " . (isset($typeDef['arrayType'])
                            ? $typeDef['arrayType'] : ''));
        ...
        if ($phpType == 'struct') {
            ...
            if (is_array($value)) {
                $elementAttrs = $this->serializeComplexTypeAttributes($typeDef,
                        $value, $ns, $uqType);
                if ($use == 'literal') {
                    if ($forceType) {
                ...
                if (isset($typeDef['simpleContent']) && $typeDef['simpleContent']
                        == 'true') {
                    if (isset($value['!'])) {
                        $xml .= $value['!'];
                    ...
                    // complexContent
                    $xml .= $this->serializeComplexTypeElements($typeDef,
                            $value, $ns, $uqType, $use, $encodingStyle);
                }
                $xml .= "</$elementName>";
            } else {
    ...
    function serializeComplexTypeAttributes($typeDef, $value, $ns, $uqType) {
        ...
        if (isset($typeDef['extensionBase'])) {
            ...
            if ($typeDefx = $this->getTypeDef($uqTypex, $nsx)) {
                ...
                $xml .= $this->serializeComplexTypeAttributes($typeDefx, $value,
                        $nsx, $uqTypex);
            } else {
     ...
     * @access private
     */
    function serializeComplexTypeElements($typeDef, $value, $ns, $uqType, $use =
    'encoded', $encodingStyle = false) {
        ...
        if (isset($typeDef['extensionBase'])) {
            ...
            if ($typeDefx = $this->getTypeDef($uqTypex, $nsx)) {
                ...
                $xml .= $this->serializeComplexTypeElements($typeDefx, $value,
                        $nsx, $uqTypex, $use, $encodingStyle);
            } else {
                $this->debug("extension base $nsx:$uqTypex is not a supported type");
            }
        }
        if (isset($typeDef['elements']) && is_array($typeDef['elements'])) {
            ...
            foreach ($typeDef['elements'] as $eName => $attrs) {
                ...
                if (isset($optionals) && (!isset($xvalue[$eName])) && ( (!isset($attrs['nillable']))
                        || $attrs['nillable'] != 'true')
                ) {
                    ...
                    if (isset($attrs['maxOccurs']) && ($attrs['maxOccurs'] == 'unbounded'
                            || $attrs['maxOccurs'] > 1) && isset($v) && is_array($v)
                        ...
                        foreach ($vv as $k => $v) {
                            if (isset($attrs['type']) || isset($attrs['ref'])) {
                                ...
                                $xml .= $this->serialize_val($v, $eName, false,
                                        false, false, false, $use);
                            }
                        }
                    } else {
                        if (is_null($v) && isset($attrs['minOccurs']) && $attrs['minOccurs']
                                == '0') {
                            // do nothing
                        } elseif (is_null($v) && isset($attrs['nillable']) && $attrs['nillable']
                                == 'true') {
    ...
    '') {
        ...
        $restrictionBase = strpos($restrictionBase, ':') ? $this->expandQname($restrictionBase)
                    : $restrictionBase;
        $arrayType = strpos($arrayType, ':') ? $this->expandQname($arrayType) : $arrayType;

        $typens = isset($this->namespaces['types']) ? $this->namespaces['types']
                    : $this->namespaces['tns'];
        $this->schemas[$typens][0]->addComplexType($name, $typeClass, $phpType,
                $compositor, $restrictionBase, $elements, $attrs, $arrayType);
    }
     ...
     * @access public 
     */
    function addOperation($name, $in = false, $out = false, $namespace = false, $soapaction =
    false, $style = 'rpc', $use = 'encoded', $documentation = '', $encodingStyle =
    '') {
        if ($use == 'encoded' && $encodingStyle == '') {
            $encodingStyle = 'http://schemas.xmlsoap.org/soap/encoding/';
        }
        ...
        if ($style == 'document') {
            ...
            $this->addComplexType($name . 'RequestType', 'complexType',
                    'struct', 'all', '', $elements);
            ...
            $this->addComplexType($name . 'ResponseType', 'complexType',
                    'struct', 'all', '', $elements);
...
class nusoap_parser extends nusoap_base {
     ...
     * @access   public
     */
    function __construct($xml, $encoding = 'UTF-8', $method = '', $decode_utf8 =
    true) {
        parent::__construct();
        $this->xml = $xml;
            ...
            if ($pos_xml !== FALSE) {
                ...
                if (preg_match("/encoding=[\"']([^\"']*)[\"']/", $xml_decl, $res)) {
                    ...
                    if (strtoupper($xml_encoding) != $encoding) {
                        ...
                        $this->debug($err);
                        if ($encoding != 'ISO-8859-1' || strtoupper($xml_encoding)
                                != 'UTF-8') {
                            $this->setError($err);
                            return;
                        }
    ...
    function start_element($parser, $name, $attrs) {
        ...
        foreach ($attrs as $key => $value) {
            ...
            // if ns declarations, add to class level array of valid namespaces
            if ($key_prefix == 'xmlns') {
                if (preg_match('/^http:\/\/www.w3.org\/[0-9]{4}\/XMLSchema$/',
                                $value)) {
                ...
                if (isset($this->message[$pos]['type']) && $this->message[$pos]['type']
                        == 'array') {
            ...
            if (isset($this->message[$pos]['attrs']['href'])) {
                ...
                if (isset($this->message[$pos]['nil']) && $this->message[$pos]['nil']) {
                    ...
                    if (isset($this->message[$pos]['type'])) {
                        ...
                        if (isset($this->message[$parent]['type']) && ($this->message[$parent]['type']
    ...
    function decodeSimple($value, $type, $typens) {
        ...
        if ($type == 'nonPositiveInteger' || $type == 'negativeInteger' || $type
                == 'nonNegativeInteger' || $type == 'positiveInteger' || $type == 'unsignedInt'
...
class nusoap_client extends nusoap_base {
     ...
     * @access   public
     */
    function __construct($endpoint, $wsdl = false, $proxyhost = false, $proxyport =
    false, $proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout =
    30, $portName = '') {
        parent::__construct();
     ...
     * @access   public
     */
    function call($operation, $params = array(), $namespace = 'http://tempuri.org', $soapAction =
    '', $headers = false, $rpcParams = null, $style = 'rpc', $use = 'encoded') {
        ...
        if ($this->endpointType == 'wsdl' && $opData = $this->getOperationData($operation)) {
            ...
            if (is_string($params)) {
                ...
                $payload = $this->wsdl->serializeRPCParameters($operation,
                        'input', $params, $this->bindingType);
            } else {
            ...
            if (is_string($params)) {
                ...
                foreach ($params as $k => $v) {
                    $payload .= $this->serialize_val($v, $k, false, false,
                            false, false, $use);
                }
            } else {
                $this->debug('params must be array or string');
        ...
        $this->debug('SOAP message length=' . strlen($soapmsg) . ' contents (max 1000 bytes)=' . substr($soapmsg,
                        0, 1000));
        // send
    ...
    function loadWSDL() {
        ...
        $this->wsdl = new wsdl('', $this->proxyhost, $this->proxyport,
                $this->proxyusername, $this->proxypassword, $this->timeout,
        ...
        switch (true) {
            ...
            case preg_match('/^http/', $this->endpoint):
                ...
                if ($err = $http->getError()) {
                    ...
                    return $this->parseResponse($http->incoming_headers,
                                    $this->responseData);
                }
                break;
            default:
    ...
    function parseResponse($headers, $data) {
        ...
        $this->debug('Use encoding: ' . $this->xml_encoding . ' when creating nusoap_parser');
        $parser = new nusoap_parser($data, $this->xml_encoding,
                $this->operation, $this->decode_utf8);
        ...
        foreach ($this->operations as $operation => $opData) {
            if ($operation != '') {
                ...
                if (sizeof($opData['input']['parts']) > 0) {
                    ...
                    $paramCommentStr = substr($paramCommentStr, 0,
                ...
                $opData['namespace'] = !isset($opData['namespace']) ? 'http://testuri.com'
                            : $opData['namespace'];
